  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      ...
      if (netState.isConnected && user?.token) {
        await fetchAndSaveLeadsByStatus(user.token, 'PricingLeads');
        console.log('[QCCompletedLeads] ✅ Refreshed from server');
      } else {
        ToastAndroid.show('Offline — showing cached data', ToastAndroid.SHORT);
      }
      await loadData();
    } catch (e) {
      console.error('[QCCompletedLeads] Refresh error:', e);
